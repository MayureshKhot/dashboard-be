...
exports.addBanquetRequest = [
  body("requesterName").isLength({ min: 1 }).trim().withMessage("Requester name must be specified."),
  body("eventDate").isISO8601().toDate().withMessage("Event date must be a valid date."),
  body("numberOfGuests").isInt({ min: 1 }).withMessage("Number of guests must be specified."),
  check("requesterName").escape(),
  check("eventDate").escape(),
  check("numberOfGuests").escape(),
  async (req, res) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        ...
        const { requesterName, eventDate, numberOfGuests, details, createdBy } = req.body;
        ...
        return apiResponse.successResponseWithData(res, "Request Added Successfully.", savedRequest);
      }
    } catch (error) {
      return apiResponse.ErrorResponse(res, error.message);
    }
